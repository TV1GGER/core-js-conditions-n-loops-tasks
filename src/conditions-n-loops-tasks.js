...
function convertToRomanNumerals(num) {
  ...
  let resultStr = '';
  if (num <= 10) {
    resultStr += numArr[num - 1];
  }
  if (num > 10 && num < 20) {
    resultStr += numArr[9] + numArr[num - 11];
  }
  if (num > 20 && num < 30) {
    resultStr += numArr[10] + numArr[num - 21];
  }
  if (num > 30 && num <= 39) {
    resultStr += numArr[11] + numArr[num - 31];
  }
  if (num === 20) {
    resultStr += numArr[10];
    return resultStr;
  ...
  for (let i = 0; i < numberStr.length; i += 1) {
    ...
    switch (numberStr[i]) {
      ...
      case '+':
        ...
        break;
      default:
        resultStr += `${numArr[Number(numberStr[i])]}`;
    }
  }
  return resultStr;
}
